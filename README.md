...
The way to resolve this (at least one way) is to have an external media with the required packages for wiki copied to it. This should be a USB device or drive **not** also used as the boot media. As everyone can have different Wifi NIC chipsets, I am going to post some general which will vary machine to machine, user to user. It is up to you to determine what Wifi chipset you are using and what the proper firmware for it is.
